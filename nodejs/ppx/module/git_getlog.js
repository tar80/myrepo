...
module.exports.diff = (args) => {
  return new Promise((resolve, reject) => {
    ...
    /* use delta */
    exec(`cd ${args.root} && git diff -U${args.diffUnified} HEAD^ | delta --zero-style=#666666 --plus-emph-style="normal 28" --file-color=yellow --file-style=plain --hunk-style=plain --keep-plus-minus-markers --width=variable > ${args.listfile}`,
      (err) => {
        if (err) {
          reject(err);
        }
