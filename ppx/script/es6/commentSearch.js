...
const g_regSearchKey = (() => {
  const msg = ABB_SEARCH ? '※空白あり(&検索), 空白と単語末尾-(-検索), 空白なし|(or検索)' : '※正規表現';
  const compPath = PPx.Extract('%su"taglist"') && ' %%%%: *completelist -set -file:%%su""taglist"" -detail:""user1""';
  let minus = '';
  let input = PPx.Extract(`%*script(%'scr'%\\compCode.js,"is","""%%","Search Comment.. ${msg}","*editmode e${compPath}")`);
  (input === '') && PPx.Quit(-1);
