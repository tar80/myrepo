...
cmd['gvim'] = function () {
  ...
  return (path, shortname, number, duplicate, search_word) => {
...
cmd['gvim_commit'] = function () {
  ...
  return function (path, shortname, number, duplicate, search_word) {
    ...
    const path_ = path.slice(root.length + 1).replace(/\\/g, '/');
    const split_word = search_word.split('@@@');
    PPx.Execute(
      `%Obd gvim --remote-send "<Cmd>tabnew ${split_word[0]}:${PPx.Extract('%*name(C,' + path_ + ')')}|set bt=nofile|execute 'silent! r! git cat-file -p ${split_word[0]}:${path_}'|0d_|${number-1} /${split_word[1]}/<CR>"`
    );
  };
};
