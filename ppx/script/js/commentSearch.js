...
var g_order = (PPx.Arguments.length && PPx.Arguments(0)) || 'filter';
var g_regSearchKey = (function () {
  var msg = ABB_SEARCH ? '※空白あり(&検索), 空白と単語末尾-(-検索), 空白なし|(or検索)' : '※正規表現';
  var compPath = PPx.Extract('%su"taglist"') && ' %%%%: *completelist -set -file:%%su""taglist"" -detail:""user1""';
  var minus = '';
  var input = PPx.Extract('%*script(%\'scr\'%\\compCode.js,"is","""%%","Search Comment.. ' + msg + '","*editmode e' + compPath + '"');
  (input === '') && PPx.Quit(-1);
  ...
  if (ABB_SEARCH) {
    input = (function () {
      ...
      return input.replace(/([^\s-]*)(.)/g, function (match, p1 ,p2) {
        var fmt = {
          ' ': '(?=.*' + p1 + ')',
          '-': '(?!.*' + p1 + ')'
        };
...
var g_hlNum = (function (n) { return isNaN(n) ? 1 : n; })(Number(g_order));

// ハイライトを初期化
PPx.Execute('*markentry -highlight:0');
for (; i--;) {
  var thisEntry2 = PPx.Entry(i);
