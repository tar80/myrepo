...
get['status'] = function (root, listfile, commit, logmax, unified) {
  PPx.Execute(
    '*cd ' + root +
    ' %: %On *ppb -c git status --porcelain -uall >' + listfile + ' %&'
  );
...
get['log'] = function (root, listfile, commit, logmax, unified) {
  PPx.Execute(
    '*cd ' + root + ' %:' +
    '%On *ppb -c git log -n' + logmax + ' --all --date=short --graph --format="%%OD%%%%h @[%%%%ad]@ %%%%d%%%%s%%OD-" >"' + listfile + ' %&'
  );
  ...
  loadADODB(listfile, function (stream, hop) {
    ...
    do {
      ...
      _sLineItemL.replace(/^(\W*)(\w*)\s@(.[0-9-]*.)@(.*)/, function (match, p1, p2, p3, p4) {
        ...
        _arrListL.push(
          '"' + p2 + '","' + p1 + '",A:H' + p5 + ',C:0.0,L:0.0,W:0.0,S:0.0,R:0.0,H:0,M:0,T:"' + p3 + p4 + '"'
        );
      });
    } while (!stream.EOS);
...
get['commit'] = function (root, listfile, commit, logmax, unified) {
  PPx.Execute(
    '*cd ' + root + ' %:' +
    '%On *ppb -c git log -n1 --date=short --name-status --format="%%OD%%%%h (%%%%cr)%%%%s %%%%d%%OD-" ' + commit + ' >"' + listfile + ' %&'
  );
  ...
  loadADODB(listfile, function (stream, hop) {
    do {
      ...
      if (_bFirstline === true) {
        ...
        _arrListC.push('"' + _sLineItemC + '","**",A:H10,C:0.0,L:0.0,W:0.0,S:0.0,R:0.0,H:0,M:0,T:"' + commit + '"');
        continue;
      }
      ...
      _sLineItemC.replace(/^(.)\t?(.*)/, function (match, p1, p2) {
        ...
        _arrListC.push(
          '"' + p2 + '","' + p1 + '",A:H' + p3 + ',C:0.0,L:0.0,W:0.0,S:0.0,R:0.0,H:0,M:0,T:"' + commit + '"'
        );
      });
    } while (!stream.EOS);
...
get['diff'] = function (root, listfile, commit, logmax, unified) {
  PPx.Execute(
    '*cd ' + root + ' %:' +
    '%On *ppb -c git diff -U' + unified + ' --diff-filter=AM --no-prefix --color-words HEAD^ >' + listfile
  );
  return;
};
...
get['diff_commit'] = function (root, listfile, commit, logmax, unified) {
  PPx.Execute(
    '*cd ' + root + ' %:' +
    '%On *ppb -c git show --no-prefix --color ' + commit + ' >' + listfile
  );
  return;
};
