...
var g_arg = PPx.Arguments.Length && PPx.Arguments(0) | 0;
...
// repository rootを取得
var repoRootDir = (function () {
  var upperDir = PPx.Extract('%*name(DCK,%1)');

  if (upperDir.indexOf('aux:') === 0) {
    upperDir = upperDir.replace(/aux:(\\\\)?S_[^\\]*\\(.*)?/, function (_match, p1, p2) {
      return p1 === '\\\\' ? '' : p2;
    });
  }
  ...
  if (upperDir !== '') {
    ...
    do {
      var hasGit = fso.BuildPath(upperDir, '.git');
...
({
  0: function () { return PPx.Result = repoRootDir + ',' + branchName; },
  1: function () {
